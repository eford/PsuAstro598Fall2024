...
# ╔═╡ 499f6bc0-6fdf-11ef-0f6c-53a115499ae8
begin
	using Distributions
...
# ╔═╡ c3ba10f8-eeaf-4fbc-82a7-83c104fa8220
md"""
# CCF Demonstration
"""

# ╔═╡ e43d4a87-d203-4b9a-b187-9390d052018c
md"""
Use Espresso G2 mask: $(@bind use_espresso CheckBox(default=true))
"""

# ╔═╡ efe48e72-aca0-402d-b2b8-c04a4705ddfc
if use_espresso
	md"""
	$nbsp $nbsp 
	Central λ to plot  $(@bind λ_central NumberField(3800:10:7800; default=6350)) Å
	$nbsp $nbsp $nbsp $nbsp 
	Δλ $(@bind Δλ_plt NumberField(1:100; default=20)) Å
	"""
else
	λ_central = 6400
	Δλ_plt = 20
	md"""
	Number of lines: $(@bind num_lines NumberField(1:100; default=5))
	$nbsp $nbsp
	$(@bind regen_spectrum Button("New spectrum"))
	"""
end

# ╔═╡ 8bc8b659-9692-4631-aefe-d3f54affc3b6
md"""
CCF v range: $(@bind v_lim Slider(1e3:1e3:1e6; default=2e5))
Mask width factor: $(@bind mask_width_factor NumberField(1:30; default=5.0))
"""

# ╔═╡ 942c6d1a-b3dc-4031-b416-20b144de1ffe
md"""
## Parameters
"""

# ╔═╡ e2d7ef4c-a1df-4e24-a642-3286bc9cbb02
begin
	scale_factor = 1.0;                # To make things easier to see
end;

# ╔═╡ 97741a67-9570-4b97-8057-8c0d953c38f1
begin
...
# ╔═╡ 63ec6405-d211-435a-a790-934b2dac7ef3
md"""
v Shift CCF Mask: $(@bind v_plt Slider(-v_lim:Δv:v_lim; default=0.0))
"""

# ╔═╡ f8ab87c1-02ef-4c84-a5f5-dc01146d55c2
begin
	mask_Δlogλ = mask_width_factor * Δv/c # width of CCF entries
	δv = Δv/2                             # m/s Spacing of CCF evaluations
end;

# ╔═╡ 0b3caaa4-ef69-4cd8-9b78-3dfe9d65c136
md"""
## Calculations
"""

# ╔═╡ 5fc9011c-19ca-41ff-a63d-1a9eed9ba80e
begin
λ_min = λ_central - Δλ_plt/2
λ_max = λ_central + Δλ_plt/2
end;

# ╔═╡ 78ab8174-5b72-4a3a-9817-02c94cf63ae7
λ = exp.(range(log(λ_min),log(λ_max),step=Δv/c));

# ╔═╡ 208c9446-88c9-44fa-a5b3-61c2ca4f712c
v_grid = range(-v_lim,stop=v_lim, step=δv);

# ╔═╡ 43dc13d9-f99d-4299-85ea-c5ea764b9f1d
md"""
## Functions
"""

# ╔═╡ cb68ca09-0de8-4cdc-9c67-a713ec120289
function mask_entry(λ::AbstractVector, λc::Real, depth::Real, Δlogλ::Real)
	ifelse.(abs.(λ.-λc) .<= Δlogλ * λc, depth, zero(depth))
end

# ╔═╡ 597b9f74-f547-4486-b511-b3f384380cca
function line(λ::AbstractVector, λc::Real, depth::Real, σ::Real)
	1.0 .- depth .* exp.(-0.5/σ^2 .* (λ.-λc).^2 )
end

# ╔═╡ 782287e9-a6b8-4ded-a1f9-b51220b93c59
...
# ╔═╡ cc90e1f9-3f2d-4aed-bfa4-5737f06b70aa
...
# ╔═╡ f030bd4a-c5ec-47a5-a75b-1f74d98f0689
...
# ╔═╡ 14a50f30-278a-4c39-ac41-4ca743cd305f
md"""
## Setup
"""

# ╔═╡ 5ee26a7e-3283-46e1-a870-1adc77897b80
TableOfContents()

# ╔═╡ 059bcc56-b5a2-4248-876a-6a075c3dcd51
begin
...
# ╔═╡ a6dbcf2f-8786-4fb4-81ee-802ef4909f48
begin
...
# ╔═╡ 0f975239-3766-42ea-a090-728c2edf8fe7
mask_mask = map(λ->λ_min<=λ<=λ_max,espresso_mask_λ);

# ╔═╡ 06fadab7-d900-4095-9a3f-210d9f06354f
if use_espresso
...
# ╔═╡ 561db85c-d42d-4cae-9d20-53588fad6f26
flux = calc_spectrum(λ,mask_λs,mask_depths,σ_logλ);

# ╔═╡ 30a4f3ce-0400-40b7-89f2-07ffbf092cc1
begin
	mask_plt = calc_mask(λ,mask_λs,mask_depths,mask_Δlogλ,v_plt);
	ccf_at_v_plt = sum(mask_plt.*flux);
end

# ╔═╡ 33988c89-037c-4cb3-b495-5f2d056d1aef
let
	plt1 = plot(xlabel="λ (nm)", ylabel="Normalized Flux or Mask", legend=:right)
	plot!(plt1,λ,flux,label="Flux")
	plt2 = plt1
	plot!(plt2,λ,mask_plt,label="Mask")
end

# ╔═╡ 48af315d-f5ac-4c74-82b3-2c95b6033167
ccf = calc_ccf(v_grid, λ, flux, mask_λs,mask_depths,mask_Δlogλ);

# ╔═╡ efd82551-e0c1-4513-a618-199306934e44
let
	plt1 = plot(xlabel="v (m/s)", ylabel="CCF(v)", legend=:none)
	plot!(plt1, v_grid,ccf, label=:none)
	scatter!(plt1, [v_plt,],[ccf_at_v_plt,], mc=1, ms=5)
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "f766c2c4fd07116cb11e11057f89e9995794fafe"
